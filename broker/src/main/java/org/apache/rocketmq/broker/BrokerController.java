...
public class BrokerController {
    ...
    public BrokerController(
        ...
        // consumerOffsetManager 管理consumer消费offset的
        // topicConfigManager 管理Topic配置的
        // pullMessageProcessor 处理Consumer发送请求过来拉取消息的
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        if (result) {
            try {
                // 创建消息存储管理组件，用来管理磁盘上的消息的
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);

                // 如果启动了dleger技术进行主从同步以及管理commitlog 需要初始化一堆dleger相关组件
        ...
        if (result) {
            // Broker 既作为Netty的服务器，也作为Netty的客户端
            // Producer发送请求来发消息
            // Consumer发送请求来拉消息
    ...
    public void start() throws Exception {
        ...
        // BrokerOuterApi是核心组件
        // 让Broker通过Netty客户端发送请求出去给别人
        // Broker发送请求到NameServer去注册以及心跳，其实都是通过这个组件
        if (this.brokerOuterAPI != null) {
